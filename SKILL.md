---
name: xuanji-mini
description: 处理已注册的 TapTap Android 下载/安装 DQC 告警；通过三个任务级 Host 工具完成确定性一级归因。
---

# 璇玑 Mini

每次只处理一份完整 DQC payload。`primary_v1` 的路由、指标定义、根预检、新增性门禁、固定队列、校验和任务组装全部由 Host 执行。
...
1. 为当前上游 request 生成稳定且唯一的 `task_id`。
...
以 Host 返回的 `analysis_preview`、`overall_status` 和紧凑 validation receipt 摘要作为任务结果。不得重算状态、重组 investigations 或用旧 run/batch 补齐证据。

完整已校验 analysis 由 Host 内部 task sink 交给上层 writer；模型可见 preview 不是可直接替代权威产物的文件。
...
- `primary_v1` 不执行二级归因、背景、错误码扩展、严格漏斗、四象限、准实验、负对照、查询并行或通用缓存。
