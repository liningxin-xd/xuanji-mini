...
部署方通过 Host 私有的 `XUANJI_ANALYSIS_PROFILE` 选择档案，不得成为模型参数。`primary_v1` 保持固定一级队列；`primary_v2` 再执行剔除反事实、一次二级归因、最多三款游戏背景查询，并按 `enhancement-priority.yaml` 用冻结证据和固定优先级选择最多两个查询增强，禁止模型选择。当前仅下载错误码可运行：只允许 `download / app / 下载失败率` 在已有一级候选、根不利变化至少 5bp 且当前受影响实体至少 100 时触发；其他路由按策略跳过，且不推导最终失败或恢复。
