...
| ID | 场景 | Expected |
|---|---|---|
| `D-01` | 游戏和预约均合法，游戏已充分解释 | 保留两个快判家族；仍完成固定五维队列，再按条件做反事实、一次二级和增强 |
...
| `D-09` | `device_brand` 或其他一级家族原始桶超过 1000 | SQL 内保留合法候选并聚合闭合残差；结果少于 250 行，源桶合并计数完整，不得使用 DView 截断结果 |
...
| `I-02` | 安装游戏家族完成，无论是否已形成合法候选 | 阶段之后均依次尝试 `brand -> channel_group -> storage -> OS -> apk_size`；各一级维度按未舍入影响平等竞争 Top 3 |
| `I-03` | 阶段或渠道查询失败、渠道无候选或只有质量桶 | 记录当前家族限制并继续后续低基数官方维度队列；不得升级为整条不支持 |
...
| `I-09` | 品牌、OS 或安装事件版本原始桶超过 1000 | 在对应模板内收敛为候选、质量和残差桶；返回 1000 行或基数审计不闭合时只淘汰当前家族/诊断并继续队列 |
| `I-10` | 单一安装渠道桶恶化，其他渠道未达到广泛性门槛 | 保留具体 `channel_group` 一级方向，不新增查询 |
| `I-11` | 至少两个其他安装渠道桶同向恶化且达到焦点变化的 50% | 复用渠道一级结果标记 `broad_change`，转向共同影响范围；不新增查询 |

## 当前执行清单验收字段

为防止只靠文案约束，每个归因家族必须产生一条内部执行记录：
...
8. 所有分桶模板的结果少于 250 行；带基数审计字段时，`collapsed_source_bucket_count` 合计严格等于 `source_bucket_count`；未单列的非质量业务源桶必须进入闭合残差。
