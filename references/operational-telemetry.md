...
The native Host emits structured JSON records to its configured process log.
Telemetry is diagnostic metadata only: it must never change task state, retry a
query, or become analytical evidence.
...
Operation schema v2 adds:

- `operation_id` and optional `error_id`;
- `analysis_profile`, `stage`, and `failure_stage`;
- the last query's bucket, ordinal, registered stage, step, and attempt;
...
Exception messages, traceback frames, module paths, local paths, SQL, DView
response content, query IDs, receipts, hashes, payloads, and credentials are
forbidden. Exception traversal is bounded by node count and depth, and records
only sanitized class names.

## Operator Procedure

1. Copy the `error_id` from the public ToolError and find the matching
   `xuanji_operation` record.
2. Use its `operation_id` to collect the operation's `xuanji_event` records.
3. Read `failure_stage`, `exception_type`, and the last-query fields before
...
4. Treat a `query_failed` event as a query-level signal. It can be converted
...
5. Retry only under the task resume contract. Keep the original task ID,
...
be added to labels or notifications.
