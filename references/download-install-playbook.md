...
告警新增性允许继续且调查属于安装链路时，在完成 `game_id` 一级家族后、决定非游戏一级和二级方向前，必须尽量复用同一批安装归因数据完成轻量阶段路由。优先把当前与基线的阶段计数合并到已有一级聚合，不单独扫描事件明细，不计入方向增强查询上限。
...
没有命中下列触发条件时立即停止，不执行验证查询，不需要逐项记录“未验证”。每个调查最多执行两个需要新增查询的增强模块；每个模块只设计一条聚合查询同时覆盖所需日期，因明确 SQL 错误按规则修正重试不视为新模块。多个需要新增查询的模块同时触发时，按“安装严格漏斗 -> 错误码与恢复 -> 跨维度候选重叠 -> 同日新旧版本准实验 -> 同类负对照”的顺序选择前两个；未命中安装或失败信号时直接跳过前两个链路专项。若安装严格漏斗与错误码使用同一已登记事件源，且一条聚合查询能同时满足两者全部门禁，则合并为一个链路专项模块、只占一个名额；不得为节省名额而省略任一模块的字段、守恒或字典门禁。因上限跳过已触发模块时写入 `evidence_limits`。不得借增强验证新增三级、多个二级父节点或无方向组合扫描。
...
- 已通过门槛的 `app_major_version`、`os_major_version`、`device_brand` 或 `storage_headroom_tier` 候选在该切片内同样出现至少 5bp 的阶段损耗增加。

该模块还必须有已登记且字段语义可确认的事件数据源，APK 能使用稳定 `chain_id`，沙盒能使用稳定 `install_round_id`，且当前与基线 cohort 已按知识库观察窗口成熟。没有登记的 action、关联键或观察窗口时停止模块，不得按名称猜测事件。
...
- 失败 PV 率相对基线上升至少 5bp，或每个受影响实体的 Retry 次数相对基线增加至少 50%；两种情况均要求同口径受影响实体不少于 100。

查询继承完整根范围，并优先在同一聚合中同时保留大盘和已定位焦点切片的对照。聚合结果对每个错误码至少产出当前与基线的错误事件数、受影响设备×游戏/chain 数、同口径业务分母、错误实体率、每个受影响实体的重复次数，以及可观测时的最终失败与后续恢复数，最终只用于校准既有输出字段。PV 上升但错误实体率稳定时，优先表述为重试放大，不得表述为影响面同比例扩大。

错误码含义必须根据事件日期使用已登记的当期客户端/系统错误字典或权威映射补充，并通过辅助维度门禁。不得按历史经验、编码外观或未登记的本地常量猜测含义；字典缺失、版本不适用或一码多义时保留原编码，将含义标记为未确认并写入 `evidence_limits`。原始 `info`、message 和 URL 只能使用脱敏后的类别，不得输出可回溯明细。
...
若游戏背景能提供发布、灰度或配置变更的精确时刻，且目标业务日同时包含变更前后样本，则在该模块的同一聚合查询中补充小时级断点对照。阶段边界必须直接取自变更记录并在查看结果前冻结，不得根据率值最低的小时反向移动断点。下载按首次下载开始小时归 cohort，安装按知识库观察窗口已成熟的下载源 cohort 归属；临近数据截止且尚未成熟的小时只报告开始量或错误信号，不与已成熟小时比较完成率。异常早于变更或新版本发布用于修复已发生问题时，不得用该版本解释异常起点。
