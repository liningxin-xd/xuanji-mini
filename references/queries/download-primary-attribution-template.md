...
仅在下载告警通过根指标复核和告警新增性门禁、准备执行一级归因时读取。模板提供 MaxCompute 安全 SQL 骨架，不定义指标；分子、分母、日期、平台和 `game_type` 必须已经由知识库与 Playbook 确认。

## 使用边界

- 每次只查询一个维度家族。`game_id` 必须使用已登记的 [下载游戏归因 QuerySpec](download-game-attribution.yaml)，不得用本模板返回未收敛的高基数游戏明细；本模板用于 `is_reserve_auto_download` 和 Playbook 后续明确选择的低基数家族。
...
- 模板中的 `__DIMENSION_SOURCE_FIELD__`、`__DIMENSION_QUALITY_SOURCE_EXPR__`、`__DIMENSION_VALUE_EXPR__` 和 `__DIMENSION_LABEL_EXPR__` 必须按 [一级归因维度登记](primary-attribution-dimensions.md) 为同一维度家族成组替换后才能执行。每次只选择当前家族的一个源字段，不得把占位符原样提交给 DView。
- 模板在 SQL 内按 Playbook 的样本、占比和质量规则保留业务桶，并把长尾聚合成 `__other_below_threshold__` 闭合残差桶。结果必须少于 250 行；禁止使用 DView 的 1000 行截断、业务 Top、`LIMIT` 或分页后继续归因。
...
WITH scoped_rows AS (
  ...
  SELECT
    ...
    SUM(CASE WHEN dt = ${business_date}
      THEN is_download_complete ELSE 0 END) AS current_numerator,
    SUM(CASE WHEN dt < ${business_date}
      THEN is_download_complete ELSE 0 END) AS baseline_numerator,
    SUM(CASE WHEN dt = ${business_date}
    ...
    SUM(CASE WHEN download_sample_flag IS NULL
          ...
          OR is_download_complete IS NULL
          OR is_download_complete NOT IN (0, 1)
...
维度替换后先静态确认：四个占位符均已消失、`GROUP BY` 与 `dimension_value` 使用逐字相同的非聚合表达式、SQL 不含任何笛卡尔积形态。查询成功后仍须按 Playbook 检查结果少于 250 行、`collapsed_source_bucket_count` 合计等于 `source_bucket_count`、未单列的非质量业务源桶进入残差、正式粒度唯一性、7 日基线完整性、分子是分母子集、各桶回勾总体以及贡献闭合。只有 `bucket_kind=dimension` 可以产生候选；匹配率和质量桶只用于提示维度来源风险，不阻止当前结果输出或后续维度继续执行。
