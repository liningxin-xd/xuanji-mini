...
安装告警通过根指标复核和告警新增性门禁后，先完成 `game_id`，再完成或按 Playbook 合法跳过独立 `D/S/C` 阶段拆解，随后必须读取本模板并按固定顺序逐个尝试全部登记的低基数家族。无论 `game_id` 是否形成合法结果、候选或已有足够解释力，都不得省略本模板或缩短队列。模板使用与安装游戏 QuerySpec 相同的官方锚点投影，不定义指标。
