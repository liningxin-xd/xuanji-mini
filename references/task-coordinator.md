...
General standalone IDs retain the bounded safe-character contract. An ID beginning
with `daily-push-` must exactly match the upstream batch-bound form:

```text
daily-push-YYYYMMDDTHHMMSSZ-<12 lowercase hex>-<64 lowercase hex request ID>
```

Malformed daily-push IDs and continuation IDs without an existing task state are
task-reference errors. The native Host rejects them before opening a DView session;
it never searches for a similar task or resolves by investigation ID.
...
The caller owns a structured continuation relay. It retains the exact task/action
identity returned by the Host and injects it into repair/finalize calls; opaque IDs
must not be regenerated by a model. Top-level and nested action identities must
match the original request before a continuation is attempted.

On the normal model-visible path, the model must not read
`contracts/dqc-routes.yaml`, routing Markdown, data-analysis knowledge-base
manifests or metric YAML, locked SQL assets, Runtime state, or the complete
Playbook. Structured identity injection does not expose any of those assets. The
Host remains the sole controller of routing, definitions, and investigation
selection, while the model still supplies the bounded repair or writer content.
